...
app.config['RATELIMIT_STORAGE_URI'] = "redis://127.0.0.1:6379/0"

# Configuración de flask-limiter para usar Redis como backend de almacenamiento
limiter = Limiter(
    key_func=get_remote_address,  # Utiliza la dirección IP del cliente para aplicar límites
    #storage_uri="redis://127.0.0.1:6379/0",  # URI de conexión a Redis (puedes cambiar el número de base de datos si es necesario)
    default_limits=["200 per day", "50 per hour"],  # Límite predeterminado para todas las rutas
    strategy="fixed-window",  # Estrategia de limitación (puede ser 'fixed-window', 'moving-window' o 'token-bucket')
    headers_enabled=True,  # Habilita los encabezados para mostrar información de rate limiting en las respuestas
    auto_check=True,  # Habilita la verificación automática de límites para todas las rutas
)

# Inicializar flask-limiter con la instancia de Flask
...
app.config['SECRET_KEY'] = '2720013bda7204720855d5c81da3be8b95c8f685696b5aa5a6af00c90f986945'  # Replace with a secure key
...
class FormulaLabForm(FlaskForm):
    petition_id = StringField('ID Petición', render_kw={'readonly': True})
    nombre = StringField('Nombre Fórmula', validators=[DataRequired(), Length(max=150)])
    descripcion = TextAreaField('Descripción', validators=[Length(max=500)])
    requiere_rele = StringField('¿Requiere RELE?', validators=[Length(max=50)])
    telefono_contacto = StringField('Teléfono de Contacto', validators=[DataRequired()])
    foto_receta = FileField('Foto de la receta', validators=[
        FileAllowed(['jpg', 'png', 'jpeg'], 'Solo se permiten imágenes')
    ])
    submit = SubmitField('Solicitar Fórmula')

    def __init__(self, *args, **kwargs):
        super(FormulaLabForm, self).__init__(*args, **kwargs)
        if not self.petition_id.data:
            # Generate unique ID using timestamp and random suffix
            timestamp = datetime.now().strftime('%Y%m%d%H%M')
            random_suffix = ''.join(random.choices(string.ascii_uppercase + string.digits, k=4))
            self.petition_id.data = f'FRM-{timestamp}-{random_suffix}'

@app.route('/lab/formulas', methods=['GET', 'POST'])
def solicitar_formula():
    app.logger.debug('Starting formula request handler')
    form = FormulaLabForm()
    if form.validate_on_submit():
        app.logger.debug(f'Form validated. Petition ID: {form.petition_id.data}')
        nueva_formula = FormulaPetition(
            petition_id=form.petition_id.data,
            nombre=form.nombre.data,
            descripcion=form.descripcion.data,
            requiere_rele=form.requiere_rele.data,
            telefono_contacto=form.telefono_contacto.data
        )

        if form.foto_receta.data:
            app.logger.debug(f'Processing image upload: {form.foto_receta.data.filename}')
            try:
                filename = secure_filename(form.foto_receta.data.filename)
                filepath = os.path.join(app.config['UPLOAD_FOLDER'], filename)
                app.logger.debug(f'Saving file to: {filepath}')
                os.makedirs(app.config['UPLOAD_FOLDER'], exist_ok=True)
                form.foto_receta.data.save(filepath)
                nueva_formula.foto_receta_path = filename
                app.logger.debug('Image saved successfully')
            except Exception as e:
                app.logger.error(f'Failed to save image: {str(e)}')
                flash(f'Error al guardar la imagen: {str(e)}', 'danger')
                return redirect(url_for('solicitar_formula'))
            
        try:
            db.session.add(nueva_formula)
            db.session.commit()
            app.logger.debug(f'Formula request {form.petition_id.data} saved to database')
            flash(f'Solicitud de fórmula {form.petition_id.data} creada exitosamente', 'success')
            return redirect(url_for('lista_formulas'))
        except Exception as e:
            app.logger.error(f'Database error: {str(e)}')
            db.session.rollback()
            flash('Error al guardar la fórmula', 'danger')
            return redirect(url_for('solicitar_formula'))
    
    app.logger.debug('Rendering formula request form')
    return render_template('lab/solicitar_formula.html', form=form)

@app.route('/lab/formulas/lista')
@login_required
@requiere_roles(RoleEnum.ADMIN.value)
def lista_formulas():
    formulas = FormulaPetition.query.order_by(FormulaPetition.fecha_solicitud.desc()).all()
    return render_template('lab/lista_formulas.html', formulas=formulas)

@app.route('/lab/formula/<petition_id>')
@login_required
@requiere_roles(RoleEnum.ADMIN.value)
def ver_formula(petition_id):
    formula = FormulaPetition.query.filter_by(petition_id=petition_id).first_or_404()
    
    # Create display dates without modifying the original datetime objects
    display_dates = {
        'fecha_preparacion': formula.fecha_preparacion.strftime('%Y-%m-%d %H:%M') if formula.fecha_preparacion else None,
        'fecha_entrega': formula.fecha_entrega.strftime('%Y-%m-%d %H:%M') if formula.fecha_entrega else None,
        'fecha_solicitud': formula.fecha_solicitud.strftime('%Y-%m-%d %H:%M') if formula.fecha_solicitud else None
    }
    
    # Get timeline activities
    activities = []
    for audit in formula.auditorias:
        activities.append({
            'tipo': 'estado',
            'fecha': audit.fecha_hora,
            'detalle': audit.detalle,
            'usuario': audit.usuario.nombre_usuario
        })
    
    if formula.notas_laboratorio:
        activities.append({
            'tipo': 'notas',
            'fecha': formula.fecha_preparacion or formula.fecha_solicitud,
            'detalle': formula.notas_laboratorio,
            'usuario': formula.preparado_por.nombre_usuario if formula.preparado_por else None
        })
    
    activities.sort(key=lambda x: x['fecha'], reverse=True)
        
    return render_template('lab/ver_formula.html', 
                         formula=formula, 
...
@app.route('/lab/formula/<petition_id>/cambiar-estado', methods=['POST'])
@login_required
@requiere_roles(RoleEnum.ADMIN.value, RoleEnum.PROFESOR.value)
def cambiar_estado_formula(petition_id):
    formula = FormulaPetition.query.filter_by(petition_id=petition_id).first_or_404()
    nuevo_estado = request.form.get('estado')
    
    if nuevo_estado:
        estado_anterior = formula.estado
        formula.estado = nuevo_estado
        
        # Update preparation/delivery dates based on status
        if nuevo_estado == 'En Preparación':
            formula.fecha_preparacion = datetime.utcnow()
            formula.preparado_por_id = current_user.id
        elif nuevo_estado == 'Completado':
            formula.fecha_entrega = datetime.utcnow()
            
        # Create audit record
        auditoria = Auditoria(
            usuario_id=current_user.id,
            accion=f'Cambio estado fórmula {petition_id}',
        ...
        notificacion = Notificacion(
            mensaje=f'Fórmula {formula.nombre} ({petition_id}) cambió a estado: {nuevo_estado}',
            usuario_id=formula.usuario_id,
            formula=formula
        )
        ...
        flash(f'Estado de fórmula actualizado a: {nuevo_estado}', 'success')
    
    return redirect(url_for('ver_formula', petition_id=petition_id))

@app.route('/lab/formula/<petition_id>/notas', methods=['POST'])
@login_required
@requiere_roles(RoleEnum.ADMIN.value, RoleEnum.PROFESOR.value)
def agregar_notas_formula(petition_id):
    formula = FormulaPetition.query.filter_by(petition_id=petition_id).first_or_404()
    notas = request.form.get('notas_laboratorio')
    
    if notas:
        formula.notas_laboratorio = notas
        
        auditoria = Auditoria(
            usuario_id=current_user.id,
            accion=f'Actualización notas fórmula {petition_id}',
            detalle='Notas de laboratorio actualizadas',
            formula=formula
        )
        ...
        flash('Notas de laboratorio actualizadas correctamente', 'success')
    
    return redirect(url_for('ver_formula', petition_id=petition_id))

@app.route('/lab/formula/<petition_id>/timeline')
@login_required
def ver_timeline_formula(petition_id):
    formula = FormulaPetition.query.filter_by(petition_id=petition_id).first_or_404()
    ...
    return render_template('lab/formula_timeline.html', formula=formula, activities=activities)

def calculate_average_preparation_time():
    completed_formulas = FormulaPetition.query.filter_by(estado='Completado').all()
    if not completed_formulas:
        return 0
    total_hours = 0
    for formula in completed_formulas:
        if formula.fecha_entrega and formula.fecha_solicitud:
            delta = formula.fecha_entrega - formula.fecha_solicitud
            total_hours += delta.total_seconds() / 3600
    return round(total_hours / len(completed_formulas), 1)

def calculate_formulas_per_day():
    today = datetime.utcnow()
    start_of_day = today.replace(hour=0, minute=0, second=0, microsecond=0)
    formulas_today = FormulaPetition.query.filter(
        FormulaPetition.fecha_solicitud >= start_of_day
    ).count()
    return formulas_today

def calculate_success_rate():
    total = FormulaPetition.query.count()
    if not total:
        return 0
    completed = FormulaPetition.query.filter_by(estado='Completado').count()
...
def registro():
    ...
    if form.validate_on_submit():
        ...
        _password=generate_password_hash(form.contrasena.data),
        rol=RoleEnum.USUARIO,
        activo=True
        )
        try:
...
def usuario_dashboard():
    ...
    formulas = FormulaPetition.query.filter_by(usuario_id=current_user.id).all()
    form = FormulaLabForm()  # Add this line
...
@app.route('/usuario/formulas')
@requiere_roles(RoleEnum.USUARIO.value)
def usuario_formulas():
    productos_asignados = Producto.query.filter_by(usuario_asignado=current_user.id).all()
    formulas = FormulaPetition.query.filter_by(usuario_id=current_user.id).all()
    form = FormulaLabForm()  # Add this line
    return render_template('usuario/formulas.html',
                         productos=productos_asignados,
                         formulas=formulas,
                         form=form)  # Add form to template context


@app.route('/usuario/solicitar_formula', methods=['POST'])
@requiere_roles(RoleEnum.USUARIO.value)
def usuario_solicitar_formula():
    form = FormulaLabForm()
    if form.validate_on_submit():
        petition_id = f"FRM-{datetime.now().strftime('%Y%m%d%H%M')}-{secrets.token_hex(2).upper()}"
        ...
        db.session.add(nueva_formula)
        db.session.commit()
        flash('Fórmula solicitada exitosamente', 'success')
        return redirect(url_for('usuario_formulas'))
...
def admin_dashboard():
    ...
    stats = {
        ...
        'formulas_pendientes': FormulaPetition.query.filter_by(estado='Pendiente').count(),
        'formulas_preparacion': FormulaPetition.query.filter_by(estado='En Preparación').count(),
        'formulas_completadas': FormulaPetition.query.filter_by(estado='Completado').count(),
        'total_formulas': FormulaPetition.query.count(),
        'tiempo_promedio': calculate_average_preparation_time(),
        'formulas_por_dia': calculate_formulas_per_day(),
        'tasa_exito': calculate_success_rate(),
        'en_cola': FormulaPetition.query.filter_by(estado='Pendiente').count()
    }
    
    formulas_recientes = FormulaPetition.query.order_by(FormulaPetition.fecha_solicitud.desc()).limit(5).all()
    
    formula = FormulaPetition.query.order_by(FormulaPetition.fecha_solicitud.desc()).first()
    activities = []
    if formula:
        for audit in formula.auditorias:
            activities.append({
    ...
    return render_template('admin/dashboard.html', 
                          ...
                          formula=formula,
                          activities=activities,
                          formulas_recientes=formulas_recientes)
